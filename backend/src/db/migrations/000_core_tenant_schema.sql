-- ============================================================
--  Migration 000: Core tenant schema (runs first — sorts before 001)
-- ============================================================
-- The CREATE TABLE statements for teachers/classes/students/attendance/
-- fees originally lived only in backend/src/db/schema.sql, which is run
-- once manually (node src/db/init.js) and was NEVER part of the migration
-- set replayed by schoolController.js's runMigrationsForSchema() when
-- provisioning a new tenant schema. Every migration from 001 onward
-- assumes these tables already exist (they ALTER them, or reference them
-- via FK) — without this file, a freshly created tenant schema has none
-- of its own core tables, and every query silently falls through
-- Postgres's search_path to the shared `public` schema instead.
-- ============================================================
